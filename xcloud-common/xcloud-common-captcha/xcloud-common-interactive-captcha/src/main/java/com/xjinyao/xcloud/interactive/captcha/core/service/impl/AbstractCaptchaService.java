...
public abstract class AbstractCaptchaService implements CaptchaService {
    ...
    protected static String waterMark = "人机行为验证";

    protected static Boolean watermarkEnable = false;
    ...
    public void init(Properties config) {
        ...
        log.info("--->>>初始化验证码底图<<<---");
        waterMark = config.getProperty(Const.CAPTCHA_WATER_MARK, "人机行为验证");
        slipOffset = config.getProperty(Const.CAPTCHA_SLIP_OFFSET, "5");
        watermarkEnable = Boolean.parseBoolean(config.getProperty(Const.CAPTCHA_WATER_ENABLE, "false"));
    ...
    public static Boolean getWatermarkEnable() {

package com.xjinyao.xcloud.common.core.redis.service;
...
public class DefaultRedisService {
    ...
    public DefaultRedisService(RedisTemplate redisTemplate) {
    ...
    public Boolean set(String key, Object value, Duration timeout) {
        ...
        try {
            ...
            if (TimeoutUtils.hasMillis(timeout)) {
                valueOperations.set(key, value, timeout.toMillis(), TimeUnit.MILLISECONDS);
            } else {
                valueOperations.set(key, value, timeout.getSeconds(), TimeUnit.SECONDS);
            }
            return true;
        } catch (Exception e) {
            log.error("Exception: {}", e.getMessage());
     ...
     * @return 对应的多个键值
     */
    public Object hmget(String key, String entriKey) {
        return hashOperations.get(key, entriKey);
    }

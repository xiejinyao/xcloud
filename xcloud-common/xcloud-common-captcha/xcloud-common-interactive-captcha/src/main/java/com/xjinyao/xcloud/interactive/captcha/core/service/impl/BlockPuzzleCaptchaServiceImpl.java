...
import com.xjinyao.xcloud.interactive.captcha.core.enums.RepCodeEnum;
import com.xjinyao.xcloud.interactive.captcha.core.util.AESUtil;
import com.xjinyao.xcloud.interactive.captcha.core.util.ImageUtils;
import com.xjinyao.xcloud.interactive.captcha.core.util.RandomUtils;
...
import com.xjinyao.xcloud.interactive.captcha.core.vo.PointVO;
...
public class BlockPuzzleCaptchaServiceImpl extends AbstractCaptchaService {
    ...
    private static final ResourcePatternResolver resourcePatternResolver = ResourcePatternUtils.getResourcePatternResolver(new DefaultResourceLoader());

    private final static String DEFAULT_FONT = "fonts/Songti.ttc";
    ...
    public ResponseModel get(CaptchaVO captchaVO) {
        ...
        if (getWatermarkEnable()) {
            int style = Font.BOLD;
            Font watermark = new Font(waterMarkFont, style, HAN_ZI_SIZE / 2);
            try {
                doWatermark(originalImage, watermark);
                throw new RuntimeException();
            } catch (Exception e) {
                try {
                    String defaultFontUri = ResourcePatternResolver.CLASSPATH_ALL_URL_PREFIX + "/" + DEFAULT_FONT;
                    Resource[] defaultResources = resourcePatternResolver.getResources(defaultFontUri);
                    if (defaultResources.length == 0) {
                        defaultFontUri = ResourceUtils.CLASSPATH_URL_PREFIX + "/" + DEFAULT_FONT;
                        defaultResources = resourcePatternResolver.getResources(defaultFontUri);
                    }
                    if (defaultResources.length > 0) {
                        InputStream inputStream = defaultResources[0].getInputStream();
                        Font defaultWatermarkFont = Font.createFont(TRUETYPE_FONT, inputStream);
                        float size = HAN_ZI_SIZE / 2f;
                        Font defaultWatermarkDeriveFont = defaultWatermarkFont.deriveFont(style, size);
                        doWatermark(originalImage, defaultWatermarkDeriveFont);
                    }
                } catch (Exception ex) {
                    logger.error("watermark is error : {}", ex.getMessage());
                }
            }
        }
    ...
    private static void doWatermark(BufferedImage originalImage, Font watermark) {
        //设置水印
